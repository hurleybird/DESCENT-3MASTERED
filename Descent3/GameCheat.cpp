...
char *LamerCheats[]={	"?E9FI=()", // gabbagabbahey
						"=-OQESN1", // motherlode
						"C<G2DAIV", // zingermans
						"%>OQB9T@", // eatangelos
						",/,JCQ/7", // alifalafel
...
// SOME FUN.
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
char *WeaponsCheat		="BH;URJH,""8DS#";
char *CloakCheat		="L_QM[=^)""@-:!(";
char *KillRobotsCheat	="KMGKKG4D""~";
char *InvulnCheat		="BBG\\Q90L""}FQC.>$";
char *FrametimeCheat	=";;24E1)]""==";
char *CameraCheat		="X2OHN*2(""K<?@!";
char *CoolTextures		="/Q/NGQ\\,""%*HD@";
char *SuicideCheat		="\\S3YGK=N""&D+#";
...
char *StopLogCheat		="GE8FHH6*""_=JJDK";	//stoprtlog
#endif
...
void DemoCheats(int key)
{
	...
	if (	!(memcmp (cryptstring,FrametimeCheat,8))	||
			!(memcmp (oldcryptstring,OLDDEMO_FrametimeCheat,8))	||
	...
	if (	!(memcmp (cryptstring,CoolTextures,8))	||
			!(memcmp (oldcryptstring,OLDDEMO_CoolTextures,8))		||
	...
	if (!(memcmp (cryptstring,WeaponsCheat,8)))
	{
		...
		#elif OEM
			Players[Player_num].weapon_flags = HAS_FLAG(LASER_INDEX)|HAS_FLAG(VAUSS_INDEX)|HAS_FLAG(MICROWAVE_INDEX)|
				HAS_FLAG(PLASMA_INDEX)|HAS_FLAG(SUPER_LASER_INDEX)|HAS_FLAG(MASSDRIVER_INDEX)|HAS_FLAG(NAPALM_INDEX)|
				HAS_FLAG(CONCUSSION_INDEX)|HAS_FLAG(HOMING_INDEX)|HAS_FLAG(IMPACTMORTAR_INDEX)|HAS_FLAG(FRAG_INDEX)|
				HAS_FLAG(GUIDED_INDEX)|HAS_FLAG(NAPALMROCKET_INDEX)|HAS_FLAG(CYCLONE_INDEX);
	...
	if (!(memcmp (cryptstring,SuicideCheat,8))){
		...
		AddHUDMessage(tb);
		ApplyDamageToPlayer(&Objects[Players[Player_num].objnum],NULL,PD_ENERGY_WEAPON,Objects[Players[Player_num].objnum].shields+10);
		memset(tb,0,20);		
	}
} 
