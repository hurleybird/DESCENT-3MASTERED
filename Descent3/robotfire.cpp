...
void WBFireAnimFrame(object* obj, otype_wb_info* static_wb, int wb_index)
{
	...
	p_dwb->wb_anim_frame += Frametime * ((static_wb->anim_end_frame[cur_mask] - static_wb->anim_start_frame[cur_mask]) / static_wb->anim_time[cur_mask]);
...
void WBFireBattery(object* obj, otype_wb_info* static_wb, int poly_wb_index, int dynamic_wb_index, float damage_scalar)
{
	...
	for (cur_m_bit = 0; cur_m_bit < pm->poly_wb[poly_wb_index].num_gps; cur_m_bit++)
	{
		if (fire_mask & (0x01 << cur_m_bit))
		{
			...
			weapon_obj = FireWeaponFromObject(obj, static_wb->gp_weapon_index[cur_m_bit], pm->poly_wb[poly_wb_index].gp_index[cur_m_bit], (static_wb->flags & WBF_FIRE_FVEC) != 0, (static_wb->flags & WBF_FIRE_TARGET) != 0);

			if (first)
			{
				first = 0;
				if (obj->type == OBJ_PLAYER && obj->id == Player_num && Weapons[static_wb->gp_weapon_index[cur_m_bit]].recoil_force > 0)
				{
	...
	if (num_fired)
	{
		...
		if (obj == Viewer_object)
			Sound_system.Play3dSound(static_wb->fm_fire_sound_index[p_dwb->cur_firing_mask], SND_PRIORITY_HIGHEST, &cur_pos, MAX_GAME_VOLUME, SIF_NO_3D_EFFECTS);
		else
	...
	if ((static_wb->flags & WBF_GUIDED) && obj->type == OBJ_PLAYER)
	{
		...
		if (num_fired > 0)
		{
			...
			if (!Game_toggles.guided_mainview && obj->id == Player_num)
			{
				...
				CreateSmallView(window, Objects[weapon_obj].handle, SVF_POPUP + SVF_BIGGER + SVF_CROSSHAIRS, 0.0, D3_DEFAULT_ZOOM, -1, TXT_HUD_GUIDED);
			}
		}
	}
	...
	if (num_fired >= 1 && obj->type == OBJ_PLAYER && obj->id == Player_num && (Game_mode & GM_MULTI) && !(Netgame.flags & NF_PERMISSABLE))
	{
