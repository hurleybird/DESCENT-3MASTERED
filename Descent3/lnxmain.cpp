...
void install_signal_handlers(void)
{
	...
	if(sigaction(SIGHUP,&fact,NULL)) fprintf(stderr,"SIG: Unable to install SIGHUP\n");
...
//put directories into database
  ...
  if (!dir) strcpy(path, loki_getdatapath());  //"/usr/local/games/descent3");
  else strcpy(path, dir);

  if (!netdir) strcpy(netpath, "");
  else strcpy(netpath, netdir);
...
static void register_d3_args(void)
{
    ...
    {
        loki_registeroption(d3ArgTable[i].lng,
                            d3ArgTable[i].sht,
                            d3ArgTable[i].comment);
    } // for
} // register_d3_args
...
static void hogfileRefresh(const char *x)
{
    printf(" - %s\n", x);
} // hogfileRefresh
...
static void check_beta()
{
    ...
    {
        fprintf(stderr,
            "Thanks for participating in the Descent 3 beta test!\n"
    ...
    {
        fprintf(stderr,
            "Warning: This is a beta version of DESCENT 3.\n"
...
*/
    ...
    snprintf(game_version_buffer, sizeof (game_version_buffer),
             "%d.%d.%d%s%s", D3_MAJORVER, D3_MINORVER, D3_BUILD, LOKI_VERSION,
             GAME_VERS_EXT);
