...
#define CANBEASSIGNEDSCRIPT(obj)	( (obj) && ( (obj->type==OBJ_ROBOT) || (obj->type==OBJ_BUILDING) || (obj->type==OBJ_POWERUP) || (obj->type==OBJ_CLUTTER) || (obj->type==OBJ_DOOR) || (obj->type==OBJ_CAMERA) || (obj->type==OBJ_DUMMY) ) )
#define CANHAVEANYSCRIPT(obj)		( CANBEASSIGNEDSCRIPT(obj) || ( obj && ( (obj->type==OBJ_DEBRIS) || (obj->type==OBJ_GHOST)) ) )
#define CANTYPEBEASSIGNEDSCRIPT(type)	( (type==OBJ_ROBOT) || (type==OBJ_BUILDING) || (type==OBJ_POWERUP) || (type==OBJ_CLUTTER) || (type==OBJ_DOOR) || (type==OBJ_CAMERA) )
//========================================================================
...
void Osiris_DumpLoadedObjects(char* file)
{
	...
	for (int j = 0; j < MAX_LOADED_MODULES; j++)
	{
		if (OSIRIS_loaded_modules[j].flags & OSIMF_INUSE && OSIRIS_loaded_modules[j].RefRoot != NULL)
		{
			...
			while (node)
			{
				sprintf(buffy, "Module: \"%s\" Objnum: %d Type: %d%s Id:%d\n", OSIRIS_loaded_modules[j].module_name, node->objnum, node->type, (node->dummy) ? "(Dummy)" : "", node->id);
...
bool Osiris_BindScriptsToObject(object* obj)
{
	...
	int real_type = (obj->type == OBJ_DUMMY) ? obj->dummy_type : obj->type;
...
bool Osiris_CallLevelEvent(int event, tOSIRISEventInfo* data)
{
	...
	if (event == EVT_AI_NOTIFY)
	{
		...
		{
		case AIN_GOAL_COMPLETE:			aux_event = EVT_AIN_GOALCOMPLETE;		break;
		case AIN_GOAL_INVALID:			aux_event = EVT_AIN_GOALFAIL;		break;
		case AIN_GOAL_FAIL:				aux_event = EVT_AIN_GOALFAIL;			break;
		case AIN_GOAL_ERROR:			aux_event = EVT_AIN_GOALFAIL;			break;
		}
	}
...
bool Osiris_CallEvent(object* obj, int event, tOSIRISEventInfo* data)
{
	...
	if (event == EVT_AI_NOTIFY)
	{
		switch (data->evt_ai_notify.notify_type)
		{
		case AIN_OBJ_KILLED:			aux_event = EVT_AIN_OBJKILLED;			break;
		case AIN_SEE_TARGET:			aux_event = EVT_AIN_SEEPLAYER;			break;
		case AIN_WHIT_OBJECT:			aux_event = EVT_AIN_WHITOBJECT;			break;
		case AIN_GOAL_COMPLETE:			aux_event = EVT_AIN_GOALCOMPLETE;		break;
		case AIN_GOAL_INVALID:			aux_event = EVT_AIN_GOALFAIL;			break;
		case AIN_GOAL_FAIL:				aux_event = EVT_AIN_GOALFAIL;			break;
		case AIN_GOAL_ERROR:			aux_event = EVT_AIN_GOALFAIL;			break;
		case AIN_MELEE_HIT:				aux_event = EVT_AIN_MELEE_HIT;			break;
		case AIN_MELEE_ATTACK_FRAME:	aux_event = EVT_AIN_MELEE_ATTACK_FRAME;	break;
		case AIN_MOVIE_START:			aux_event = EVT_AIN_MOVIE_START;		break;
		case AIN_MOVIE_END:				aux_event = EVT_AIN_MOVIE_END;			break;
		}
	}
...
inline int FORM_HANDLE(int counter, int slot)
{
	return (((counter & 0xFFFFFF) << 8) | (slot & 0xFF));
}

inline int GET_SLOT(int handle)
{
	return (handle & 0xFF);
}
...
bool Osiris_RestoreSystemState(CFILE* file)
{
	...
	if (read_module_count != loaded_module_count)
	{
		...
		//when this happens.
		mprintf((0, "OSIRIS: Restoring global state, the number of loaded modules is not the same as the restored count (%d vs. %d)\n", loaded_module_count, read_module_count));
...
{
	Osiris_ClearExtractedScripts(false);
}
...
// Returns -1 if there isn't enough available memory
// Returns -2 if the unique identifier passed in is already used, but the requested amount_of_memory is different
// Returns -3 if the unique identifier passed in is already used, same size requested
