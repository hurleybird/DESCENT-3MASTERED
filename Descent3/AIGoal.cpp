...
//				mprintf((0, "Obj  %f, %f, %f\n", XYZ(&obj->pos)));
	...
	if(cur_goal)
	{
		...
		if(cur_goal->type == AIG_FOLLOW_PATH)
		{
			...
			{
				AIPathSetAsStaticPath(obj, cur_goal, cur_goal->g_info.id, cur_goal->g_info.static_path_info.start_node, cur_goal->g_info.static_path_info.end_node, cur_goal->g_info.static_path_info.cur_node);
			}
		}
...
/*			else if((cur_goal->flags & GF_HAS_PATH) && ai_info->path.num_paths != 0)
		...
		else if((Gametime > cur_goal->next_path_time || ai_info->path.num_paths == 0) && (cur_goal->type != AIG_FOLLOW_PATH))
		{
...
//				if(cur_goal->g_info.pos != goal_obj->pos)
			...
			if(f_make_path)
			{
				AIPathAllocPath(obj, ai_info, cur_goal, &obj->roomnum, &obj->pos, &roomnum, posp, 0.0f, 0, obj->handle, ignore_obj);
			}
...
void GoalPathComplete(object *obj)
{
	...
	if(COMPLETEATOBJ(cur_goal))
	{
		...
		if((goal_obj) && (cur_goal->g_info.pos != goal_obj->pos))
		{
			...
			if(AIPathAllocPath(obj, ai_info, cur_goal, &obj->roomnum, &obj->pos, &goal_obj->roomnum, &goal_obj->pos, 0.0f, 0, obj->handle, ignore_obj))
			{
				cur_goal->next_path_time = Gametime + MIN_NEXT_PATH_INTERVAL + ps_rand()/(float)RAND_MAX;
			}
		}
...
int GoalAllocSlot(object *obj, int level, float influence)
{
	...
	ASSERT((level >= 0 && level < NUM_ACTIVATION_LEVELS) || 
...
int GoalAddGoal(object *obj, unsigned int goal_type, void *arg_struct, int level, float influence, int f_goal, int guid, char subtype)
{
	ai_frame *ai_info = obj->ai_info;
	...
	switch(goal_type)
	{
		...
		case AIG_PLACE_OBJ_ON_OBJ:
		{
			...
			if(goal_obj)
			{
				...
				if(AIPathAllocPath(obj, ai_info, &ai_info->goals[goal_index], &obj->roomnum, &obj->pos, &goal_obj->roomnum, &goal_obj->pos, 0.0f, 0, obj->handle, ignore_obj))
				{
					goal_ptr->next_path_time = Gametime + MIN_NEXT_PATH_INTERVAL + ps_rand()/(float)RAND_MAX;
				}
				else
		...
		case AIG_FIRE_AT_OBJ:
		{
			...
			if((ai_info->animation_type == AS_ALERT && !(ai_info->next_animation_type == AS_FLINCH)) || ai_info->animation_type == AS_RANGED_RECOIL)
			{
		...
		case AIG_DO_MELEE_ANIM:
		{
			...
			if((ai_info->animation_type == AS_ALERT)      || 
				(ai_info->animation_type == AS_MELEE1 + 1) || 
...
//			GoalAddEnabler(obj, 1, AIE_LTE_LAST_SEE_TARGET_INTERVAL, (void *)&time, 1.0, 0.0);
		...
		case AIT_EVADER2:
		{
			...
			int gi = GoalAddGoal(obj, AIG_GET_AROUND_OBJ, (void *)&handle, ACTIVATION_BLEND_LEVEL, 3.0f, GF_NONFLUSHABLE|GF_OBJ_IS_TARGET);
			if(gi != -1) {	//DAJ -1FIX
		...
		case AIT_MELEE1:
		{
			...
			GoalAddGoal(obj, AIG_MOVE_RELATIVE_OBJ, (void *)&g_info, 1, 1.0f, GF_NONFLUSHABLE | GF_OBJ_IS_TARGET | GF_KEEP_AT_COMPLETION);
			GoalAddEnabler(obj, 1, AIE_GT_AWARENESS, (void *)&aware, 1.0, 0.0);
			GoalAddEnabler(obj, 1, AIE_LTE_LAST_SEE_TARGET_INTERVAL, (void *)&time, 1.0, 0.0);

			GoalAddGoal(obj, AIG_GET_TO_OBJ, (void *) &ai_info->target_handle, 2, 1.0f, GF_SPEED_ATTACK | GF_NONFLUSHABLE | GF_OBJ_IS_TARGET | GF_USE_BLINE_IF_SEES_GOAL | GF_KEEP_AT_COMPLETION);
...
bool GoalIsGoalEnabled(object *obj, int goal_index)
{
	...
	if(OBJGOAL(cur_goal))
	{
		...
		if(!gobj || gobj->type == OBJ_GHOST || (gobj->flags & (OF_DYING | OF_DESTROYED)) || (gobj->type == OBJ_PLAYER && Players[gobj->id].flags & (PLAYER_FLAGS_DEAD + PLAYER_FLAGS_DYING)))
		{
			return false;
		}
	}
	...
	for(i = 0; i < cur_goal->num_enablers; i++)
	{
		...
		switch(e_info->enabler_type)
		{
			...
			case(AIE_LTE_LAST_SEE_TARGET_INTERVAL):
				{
					f_enabled = (Gametime - ai_info->last_see_target_time <= e_info->time) || (Gametime - ai_info->last_hear_target_time <= e_info->time);
				}
				break;
			case(AIE_GT_LAST_SEE_TARGET_INTERVAL):
				{
