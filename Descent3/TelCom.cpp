...
void TelComFree()
{
	bm_DestroyChunkedBitmap(&Telcom_bitmap);
}
...
void FreeViewports()
{
	...
	if (windowmap.wm)
	{
		...
		{
			if (windowmap.wm[count].lt_bmp > BAD_BITMAP_HANDLE) bm_FreeBitmap(windowmap.wm[count].lt_bmp);
			if (windowmap.wm[count].rt_bmp > BAD_BITMAP_HANDLE) bm_FreeBitmap(windowmap.wm[count].rt_bmp);
			if (windowmap.wm[count].lb_bmp > BAD_BITMAP_HANDLE) bm_FreeBitmap(windowmap.wm[count].lb_bmp);
			if (windowmap.wm[count].rb_bmp > BAD_BITMAP_HANDLE) bm_FreeBitmap(windowmap.wm[count].rb_bmp);
		}
	}
}
...
int TCMMButtonDesc[] = { TXI_TCMM_BRIEFINGS,
							TXI_TCMM_CARGO,
							TXI_TCMM_AUTOMAP,
							TXI_TCMM_GOALS };
...
void TCMainMenuCallback(int efxnum)
{
	mainmenu_system = efxnum;
}
...
void TelcomRenderInit(void)
{
	for (int monitor = 0; monitor < MAX_MONITOR; monitor++)
	{
		...
		Telcom_system.Monitor_coords[monitor].right -= windowmap.wm[monitor].x + windowmap.wm[monitor].width - windowmap.wm[monitor].r_end_x;
		Telcom_system.Monitor_coords[monitor].top += windowmap.wm[monitor].t_top_y - windowmap.wm[monitor].y;
		Telcom_system.Monitor_coords[monitor].bottom -= windowmap.wm[monitor].y + windowmap.wm[monitor].height - windowmap.wm[monitor].b_bottom_y;
	}
...
void TelcomLoadHiLites(char* filelist[], int monitor, int xoff, int yoff)
{
	...
	while (filelist[TelcomHiLiteCount[monitor]]) TelcomHiLiteCount[monitor]++;
...
void TelcomRenderMouse(void)
{
	...
	if (TC_cursor > -1)
	{
		...
		rend_DrawScaledBitmap(mx, my, mx + (int)((float)cur_w * u1), my + (int)((float)cur_h * v1), TC_cursor, u0, v0, u1, v1);
	}
	rend_SetZBufferState(1);
}
...
void TelComHandleAllEvents(tTelComInfo* tcs)
{
	...
	if (!buttons && Telcom_system.Telcom_mouse_downtime > 0 && (x >= HotSpotL(POWER_BUTTON)) && (x <= HotSpotR(POWER_BUTTON)) && (y >= HotSpotT(POWER_BUTTON)) && (y <= HotSpotB(POWER_BUTTON)))
		Telcom_system.state = TCS_POWEROFF;
...
void TelComHandleKeyPress(int key, bool click, int screen_id)
{
	...
	case TCSYS_DOWN:
	{
		...
		if (TCEffects[curr_focus].type != EFX_BUTTON &&
			TCEffects[curr_focus].type != EFX_TEXT_STATIC &&
			TCEffects[curr_focus].type != EFX_TEXT_TYPE &&
		...
		if (TCEffects[curr_focus].type == EFX_BUTTON &&
			TCEffects[curr_focus].buttoninfo.button_type != BUTT_UPARROW &&
		...
		if (TCEffects[curr_focus].type == EFX_BUTTON)
		{
			...
			if (parent_id == -1 || (TCEffects[parent_id].type != EFX_TEXT_STATIC &&
				TCEffects[parent_id].type != EFX_TEXT_TYPE &&
		...
		ASSERT(TCEffects[curr_focus].type == EFX_TEXT_STATIC ||
			TCEffects[curr_focus].type == EFX_TEXT_TYPE ||
			TCEffects[curr_focus].type == EFX_TEXT_FADE);
...
float myrand(float max)
{
	return (max * (((float)ps_rand()) / ((float)RAND_MAX)));
}
...
char* TCSoundFiles[] = { "Briefstartup1","BriefStatic","Briefmonitoroff1","Briefingrunning","Briefbulb1","Briefingtype","Menu Slider Click" };
...
void TelcomPageAllIn(void)
{
	...
	const float TELCOM_SOUND_PORTION = 0.3f,
		TELCOM_BITMAP_PORTION = 0.3f,
		TELCOM_TEXTUREHI_PORTION = 0.2f,
...
tShipInfo TCSSInfo[MAX_NUM_SHIPS] =
{
	{TXI_SS_PY_SPEED,TXI_SS_PY_MANEUVERABILITY,TXI_SS_PY_SHIELDS,TXI_SS_PY_ORDANCE,TXI_SS_PY_WINGSPAN,TXI_SS_PY_LENGTH,TXI_SS_PY_HEIGHT,TXI_SS_PY_WEIGHT},
	{TXI_SS_PH_SPEED,TXI_SS_PH_MANEUVERABILITY,TXI_SS_PH_SHIELDS,TXI_SS_PH_ORDANCE,TXI_SS_PH_WINGSPAN,TXI_SS_PH_LENGTH,TXI_SS_PH_HEIGHT,TXI_SS_PH_WEIGHT},
	{TXI_SS_M_SPEED,TXI_SS_M_MANEUVERABILITY,TXI_SS_M_SHIELDS,TXI_SS_M_ORDANCE,TXI_SS_M_WINGSPAN,TXI_SS_M_LENGTH,TXI_SS_M_HEIGHT,TXI_SS_M_WEIGHT}
};
...
bool TelComIsActive(void)
{
	return Telcom_intelcom;
}
...
void TCSSSCallback(void)
{
	...
	DrawPolygonModel(&view_pos, &view_orient, TCShipSelect.ship_model, normalized_time, 0, &light_vec, light_scalar, light_scalar, light_scalar);
	...
	for (int q = 0; q < 8; q++)
	{
		...
		grtext_Printf(x, y, linebuffer); y += height;
	}
