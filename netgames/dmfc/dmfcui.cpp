...
{	
	...
	for(i=0;i<DLLMAX_TEAMS;i++){
		if(selected_num[i]!=-1 && basethis->CheckPlayerNum(selected_num[i])){
			...
			if( ( (TeamDlgFinalTeamSettings[selected_num[i]]==TS_NOTINGAME)&&(basethis->GetPlayerTeam(selected_num[i])==i)) ||
				( TeamDlgFinalTeamSettings[selected_num[i]]==i) ){
				//this player was on this team last update, is still on the team, is a valid player in the game and was selected
...
void DMFCBase::DoDMFCUITeamPlacement(bool clients_wait,bool called_by_level_start)
{
	...
	char *team_icon_names[DLLMAX_TEAMS] = {"toredteamicon.ogf","toblueteamicon.ogf","togreenteamicon.ogf","toyellowteamicon.ogf"};
/*
...
*/
	...
	//////////////////////////////////////////////////
	//Text Items
	void *start_game_text_off= DLLCreateNewUITextItem((called_by_level_start)?DTXT_UI_STARTGAME:DTXT_UI_RETURNTOGAME,UICOL_HOTSPOT_LO,-1);
	void *start_game_text_on= DLLCreateNewUITextItem((called_by_level_start)?DTXT_UI_STARTGAME:DTXT_UI_RETURNTOGAME,UICOL_HOTSPOT_HI,-1);
	...
	void *start_game_hs		= DLLHotSpotCreate(main_wnd,UID_OK,K_ENTER,start_game_text_off,start_game_text_on,40,384 - OKCANCEL_YOFFSET,130,15,UIF_CENTER);
	void *window_title_text = DLLTextCreate(main_wnd,window_title,0,7,UIF_CENTER|UIF_FIT);	
	void *client_wait_cb;
	
	if(count>2)
	{
		client_wait_cb	= DLLCheckBoxCreate(main_wnd,128,client_wait_txt,256-(DLLGetUIItemWidth(client_wait_txt)/2),192 - (DLLGetUIItemHeight(client_wait_txt)/2),130,15,UIF_FIT);
		cury = 60;
	}else
	{
	...
	case 4:
		...
		for(i=0;i<count-1;i++){
			icon_hs[YELLOW_TEAM][i] = DLLHotSpotCreate(main_wnd,id_list_ptr[i],-1,icon_b_hs[YELLOW_TEAM][i],icon_b_hs[YELLOW_TEAM][i],right_x,curr_y,icon_width,icon_height,0);
	...
	case 3:
		...
		for(i=0;i<count-1;i++){
			icon_hs[GREEN_TEAM][i] = DLLHotSpotCreate(main_wnd,id_list_ptr[i],-1,icon_b_hs[GREEN_TEAM][i],icon_b_hs[GREEN_TEAM][i],left_x,curr_y,icon_width,icon_height,0);
			curr_y += icon_height + 3;
		}
		...
		for(i=0;i<count-1;i++){
			icon_hs[BLUE_TEAM][i] = DLLHotSpotCreate(main_wnd,id_list_ptr[i],-1,icon_b_hs[BLUE_TEAM][i],icon_b_hs[BLUE_TEAM][i],right_x,curr_y,icon_width,icon_height,0);
			curr_y += icon_height + 3;
		}
		...
		for(i=0;i<count-1;i++){
			icon_hs[RED_TEAM][i] = DLLHotSpotCreate(main_wnd,id_list_ptr[i],-1,icon_b_hs[RED_TEAM][i],icon_b_hs[RED_TEAM][i],left_x,curr_y,icon_width,icon_height,0);			
			curr_y += icon_height + 3;
		}
...
void DMFCBase::DoDMFCUIWaitForPlayers(bool clients_wait)
{
	...
	void *start_game_hs		= DLLHotSpotCreate(main_wnd,UID_OK,K_ENTER,start_game_text_off,start_game_text_on,40,384 - OKCANCEL_YOFFSET,130,15,UIF_CENTER);
	void *window_title_text = DLLTextCreate(main_wnd,window_title,0,7,UIF_CENTER|UIF_FIT);	
	void *client_wait_cb	= DLLCheckBoxCreate(main_wnd,128,client_wait_txt,30,369 - OKCANCEL_YOFFSET,130,15,UIF_FIT|UIF_CENTER);
	...
	l_x = l_x+l_w+10;
	l_y = (l_y+(l_h/2))-15;
	void *kick_hs			= DLLHotSpotCreate(main_wnd,10,-1,kick_text_off,kick_text_on,l_x,l_y,130,15,UIF_FIT); l_y+=15;
