...
void DetermineScore(int precord_num,int column_num,char *buffer,int buffer_size)
{
	...
	sprintf(buffer,"%d[%d]",	pr->dstats.kills[DSTAT_LEVEL]-pr->dstats.suicides[DSTAT_LEVEL],
...
void OnClientPlayerKilled(object *killer_obj,int victim_pnum)
{
	...
	if(kpnum==victim_pnum && victim_pnum!=-1)
		...
		if((kpnum!=-1)&&(dPlayers[kpnum].team!=-1))
	...
	for(int team=0;team<NUM_TEAMS;team++){
		...
		for(int index=0;index<MAX_PLAYER_RECORDS;index++){
			...
			if(pr && pr->state!=STATE_EMPTY){
				...
				//valid player
				DLLgrtext_Printf(NameCol,y,"%s %d[%d]",pr->callsign,pr->dstats.kills[DSTAT_LEVEL]-pr->dstats.suicides[DSTAT_LEVEL],pr->dstats.kills[DSTAT_OVERALL]-pr->dstats.suicides[DSTAT_OVERALL]);
...
void SaveStatsToFile(char *filename)
{
	...
	for(p=0;p<NUM_TEAMS;p++){
		...
		sprintf(tempbuffer,TXT_TEAM,DMFCBase->GetTeamString(team_i));
	...
	for(int t=0;t<NUM_TEAMS;t++){
		...
		sprintf(buffer,TXT_TEAM,DMFCBase->GetTeamString(team_i));
		strcat(buffer,":");
		DLLcf_WriteString(file,buffer);
...
//	char name[30];
	...
	for(int i=0;i<NUM_TEAMS;i++){
		...
		sprintf(team_name,"[%d]%s",num_players,DMFCBase->GetTeamString(team));
