...
#ifdef __LINUX__	//gcc has a hard time tracing through the dervived classes of newuiButton and thinks it can't access OnFormat()
public:
#else
protected:
#endif
...
//	optional key, when pressed, triggers hotspot.
	void Create(UIWindow *wnd, int id, int key, UIItem *itemoff, UIItem *itemon, int x, int y, int w, int h, int flags=0); 
...
protected:
	...
	UIItem *GetStateItem(int state) {			
		ASSERT(state < UI_BTS_NUM); return m_Items[state];
	};

	int GetColor(int state) const { 
		ASSERT(state < UI_BTS_NUM); return m_Colors[state];
	};
...
public:
	UIGroup();
	~UIGroup();
	void Create(UIWindow *parent, char *label, int x, int y, int w, int h, ddgr_color label_color=GR_WHITE,ddgr_color box_color=GR_WHITE,int flags=0);
...
//	settings
	...
	void SetPos(int pos) {							// sets the position of the slider.
		if (pos < 0) pos = 0;
		else if (pos >= m_Range) pos = m_Range-1;
		m_Pos = pos;
	};
...
inline bool UIGadget::HasFocus() const 
{ 
