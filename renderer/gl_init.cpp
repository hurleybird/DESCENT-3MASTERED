...
int GL4Renderer::Setup(SDL_Window* window)
{
	...
	{
		Error("GL4Renderer::Setup: SDL_GL_MakeCurrent failed!\n%s", SDL_GetError());
...
int GL4Renderer::Init(oeApplication* app, renderer_preferred_state* pref_state)
{
	...
	*/
	...
	blitshader_uv_origin = blitshader.FindUniform("uv_origin");
	blitshader_uv_scale = blitshader.FindUniform("uv_scale");
	...
	if (downsampleshader_gamma == -1)
		Error("GLRenderer::Init: Failed to find downsample gamma uniform!");
	...
	motionblur_player_translation_delta = motionblurshader.FindUniform("player_translation_delta");
	...
	if (motionblur_color_source == -1 || motionblur_velocity_source == -1 ||
		...
		motionblur_has_dynamic_velocity == -1 || motionblur_player_translation_delta == -1)
