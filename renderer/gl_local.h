...
class GL4Renderer : public IRenderer
{
	...
	float captured_scene_inverse_modelview[16] = {};
	float frozen_static_motion_projection[16] = {};
	float frozen_static_motion_inverse_modelview[16] = {};
	float frozen_static_motion_previous_view_projection[16] = {};
	...
	bool captured_scene_inverse_modelview_valid = false;
	bool frozen_static_motion_valid = false;
	bool deferred_bloom_composite_pending = false;
	bool deferred_bloom_apply_pending = false;
	Framebuffer* deferred_bloom_source_framebuffer = nullptr;
	...
	GLint motionvectordebug_depth_source = -1;
	GLint motionvectordebug_object_id_source = -1;
	GLint motionvectordebug_uv_origin = -1;
	GLint motionvectordebug_uv_scale = -1;
	GLint motionvectordebug_screen_size = -1;
	GLint motionvectordebug_current_projection = -1;
	GLint motionvectordebug_current_inverse_modelview = -1;
	GLint motionvectordebug_previous_view_projection = -1;
	GLint motionvectordebug_has_static_reconstruction = -1;
	GLint motionvectordebug_has_dynamic_velocity = -1;
	...
	GLuint drawbuffer = 0;
	GLuint retained_draw_buffer = 0;
	GLsizeiptr retained_draw_buffer_stride = 0;
	GLuint retained_draw_buffer_cursor = 0;
	void* retained_draw_buffer_map = nullptr;
	int retained_draw_buffer_active_segment = -1;
	GLsync retained_draw_buffer_sync_points[GL4_DRAW_BUFFER_SYNC_POINT_COUNT] = {};
...
private:
	...
	void InitRetainedDrawBuffer();
	void DestroyRetainedDrawBuffer();
	void UseLegacyRetainedDrawBlock();
	...
	bool PixelMotionVectorConsumerActive() const;
	...
	bool CurrentDrawWritesMotionObjectId() const;
...
public:
	...
	void SetDepthSnapshotInvalidation(bool enabled) override
	{
		depth_snapshot_invalidation_enabled = enabled;
	}
