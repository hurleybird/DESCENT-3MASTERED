...
layout(set = 0, binding = 4) uniform texture2D ssaa_intermediate_texture;

void main()
{
    vec2 source_uv = PostPrimaryUv(in_uv);
    vec4 filtered;
    if (post.frame_branch.y == POST_SOURCE_SSAA_INTERMEDIATE_2X_2D)
        filtered = PostWronskiFilterAtUv(ssaa_intermediate_texture, source_uv);
    else if (post.frame_branch.y == POST_SOURCE_MSAA_RESOLVED_2D)
