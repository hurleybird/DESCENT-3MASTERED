...
vec3 FetchViewPosition(vec2 canonical_uv)
{
...
vec3 ReconstructNormal(vec2 uv, vec3 position)
{
    ...
    // Canonical +Y points down, so bottom/top are supplied in the order that
    // reproduces GL4's legacy cross(dx,dy) before its camera-facing negate.
...
float FetchAoWeight(vec2 uv)
{
    vec2 depth_uv = clamp(uv, vec2(0.0),
        vec2(1.0) - post.ao_screen_size_inv_size.zw * 0.5);
    return clamp(texture(sampler2D(depth_weight_image, post_samplers[6]),
        depth_uv).g, 0.0, 1.0);
}

float ComputeHorizon(vec3 position, vec3 normal, vec2 legacy_direction,
    float step_pixels, float jitter, float side)
{
    float ray_pixels = jitter * step_pixels + 1.0;
    float horizon = 0.0;
    float average = 0.0;
    int steps = max(int(post.sample_counts.w), 1);
    vec2 canonical_direction = vec2(legacy_direction.x, -legacy_direction.y);
    for (int step_index = 0; step_index < steps; ++step_index)
    {
        vec2 sample_uv = in_uv + round(ray_pixels * canonical_direction * side) *
...
void main()
{
    vec3 position = FetchViewPosition(in_uv);
    float far_z = post.near_far_radius_radius_pixels.y;
    ...
    vec3 normal = ReconstructNormal(in_uv, position);
    int steps = max(int(post.sample_counts.w), 1);
        ...
        direction_index < gtao_directions; ++direction_index)
    {
        ...
        ao += ComputeHorizon(position, normal, direction, step_pixels, jitter, 1.0);
        ao += ComputeHorizon(position, normal, direction, step_pixels, jitter, -1.0);
    }
