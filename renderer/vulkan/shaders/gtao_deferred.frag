...
void main()
{
	vec4 final_color = texture(
		sampler2D(final_color_image, post_samplers[0]), PostPrimaryUv(in_uv));
	...
	vec3 ao_scene_color = texture(
		sampler2D(ao_scene_image, post_samplers[0]), in_uv).rgb;
    ...
    if (PostFeature(POST_USE_PROTECTION_MASK))
    {
        ...
        ivec2 mask_pixel = ivec2(clamp(PostSecondaryUv(in_uv), vec2(0.0),
    ...
    if (post.integer_params.y != 0)
    {
        out_color = vec4(mix(final_color.rgb, ao_scene_color, scene_mask),
            final_color.a);
        return;
    }
