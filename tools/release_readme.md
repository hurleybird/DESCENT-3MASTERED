# Piccu Experiment Milestone 4

Piccu Experiment Milestone 4 is an experimental fork of Piccu Engine focused on advanced rendering features that add to the original Descent 3 presentation rather than replacing it, along with quality-of-life improvements for modern play.

Major additions in this build include:

- Supersampled anti-aliasing (SSAA) for cleaner image quality
- Per-pixel lighting toggle
- Bloom and GTAO post-process toggles
- Headphone HRTF Positional Audio

Command line options added in this build:

- `-hudtextscale <scale>` or `-hud-text-scale <scale>`: Sets HUD text scale. Values are clamped to `0.75`-`1.5`. Example: `-hudtextscale 1.25`
- `-noaltenter`: Disables Alt+Enter fullscreen/windowed switching.
- `-bloomthreshold <value>` or `-bloom-threshold <value>`: Sets the bloom threshold. Values are clamped to `0.0`-`1.0`; higher values require brighter pixels to bloom.
- `-bloomintensity <value>` or `-bloom-intensity <value>`: Sets bloom intensity. Values are clamped to `0.0`-`1.0`.
- `-glcore`, `-gl4`, `-gl45`, or `-openglcore`: Forces the Core OpenGL renderer profile.
- `-glcompat`, `-gl1`, or `-openglcompat`: Forces the compatibility OpenGL renderer profile.
- `-screenshottga` or `-screenshot-tga`: Saves screenshots as TGA files instead of PNG files.
