...
void AI_SafeSetType(int obj_handle, int ai_type)
{
	...
	switch(ai_type)
	{
		case AIT_STALKER:
		{
			...
			AI_AddGoal(obj_handle, AIG_WANDER_AROUND, 1, 1.0f, -1, GF_NONFLUSHABLE | GF_KEEP_AT_COMPLETION, &goal_pos, goal_room);
		}
		break;
		...
		case AIT_EVADER2:
		{
			...
			int gi = AI_AddGoal(obj_handle, AIG_GET_AROUND_OBJ, ACTIVATION_BLEND_LEVEL, 3.0f, -1, GF_NONFLUSHABLE|GF_OBJ_IS_TARGET, OBJECT_HANDLE_NONE);
		...
		case AIT_EVADER1:
		{
			...
			AI_AddGoal(obj_handle, AIG_MOVE_RELATIVE_OBJ, 1, 1.0f, -1, GF_OBJ_IS_TARGET | GF_SPEED_ATTACK | GF_NONFLUSHABLE, OBJECT_HANDLE_NONE);
			AI_GoalAddEnabler(obj_handle, 1, AIE_GT_AWARENESS, 1.0, 0.0, &aware);
			AI_GoalAddEnabler(obj_handle, 1, AIE_LTE_LAST_SEE_TARGET_INTERVAL, 1.0, 0.0, &time);
		...
		case AIT_MELEE1:
		{
			...
			AI_AddGoal(obj_handle, AIG_MOVE_RELATIVE_OBJ, 1, 1.0f, -1, GF_NONFLUSHABLE | GF_OBJ_IS_TARGET | GF_KEEP_AT_COMPLETION, OBJECT_HANDLE_NONE);
			AI_GoalAddEnabler(obj_handle, 1, AIE_GT_AWARENESS, 1.0, 0.0, &aware);
			AI_GoalAddEnabler(obj_handle, 1, AIE_LTE_LAST_SEE_TARGET_INTERVAL, 1.0, 0.0, &time);

			AI_AddGoal(obj_handle, AIG_GET_TO_OBJ, 2, 1.0f, -1, GF_SPEED_ATTACK | GF_NONFLUSHABLE | GF_OBJ_IS_TARGET | GF_USE_BLINE_IF_SEES_GOAL | GF_KEEP_AT_COMPLETION, OBJECT_HANDLE_NONE);
			...
			m_goal = AI_AddGoal(obj_handle, AIG_MELEE_TARGET, ACTIVATION_BLEND_LEVEL, 1.0f, -1, GF_NONFLUSHABLE | GF_OBJ_IS_TARGET | GF_KEEP_AT_COMPLETION, OBJECT_HANDLE_NONE);
...
int TurnOnSpew(int objref,int gunpoint,int effect_type,float mass,float drag,int gravity_type,ubyte isreal,float lifetime,float interval,float longevity,float size,float speed,ubyte random)
{
...
int CreateAndAttach(int me, char *child_name, ubyte child_type, char parent_ap, char child_ap, bool f_aligned = true, bool f_set_parent = false)
{
...
float TraverseShotPath(tShotData *shot_data)
{
	...
	while(!done)
	{
		...
		flags = FQ_CHECK_OBJS | FQ_IGNORE_POWERUPS | FQ_IGNORE_WEAPONS | FQ_IGNORE_MOVING_OBJECTS | FQ_IGNORE_NON_LIGHTMAP_OBJECTS;
		fate = FVI_RayCast(shot_data->object_handle, &start_pos, &target_pos, start_room, shot_data->shot_radius, flags, &ray);
...
{
	{ID_ALIENORGANISM, "AlienOrganism"},
	{ID_HEAVYTROOPER, "HeavyTrooper"},
	{ID_LIFTER, "Lifter"},
	{ID_ALIENBOSS, "AlienBoss"},
	{ID_SECURITYCAMERA, "SecurityCamera"},
	{ID_CROWDCONTROL, "CrowdControl"}
};
...
private:
	...
	void ProjectBeam(int me, int pos_handle, vector *start_pos, int start_room, vector *beam_dir, matrix *orient, int beam_flag);
...
char *AB_WanderRoomNames[AB_NUM_WANDER_ROOMS] = {
	"BossRoomA",
	"BossTunnelAB",
	"BossRoomB",
	"BossTunnelBC",
...
bool AlienOrganism::ReceiveCommand(int me, int it, char command, void *ptr)
{
	switch(command)
	{
		...
		case ALIEN_COM_HUNT_TO_OBJ:
		{
			...
			if(memory->mode != ALIEN_LANDED_RESTING) return false;
...
bool AlienOrganism::IsLandedMode(int mode)
{
	if(mode==ALIEN_LANDED
		...
		|| mode==ALIEN_LANDED_HEALING
		|| mode==ALIEN_LANDED_DEPOSITING
		|| mode==ALIEN_LANDED_WITHDRAWING) return true;
...
bool AlienOrganism::ModeIsAttackSensitive(int mode, int pending_mode)
{
	...
	if(mode != ALIEN_ATTACKING_MELEE && mode != ALIEN_ATTACKING_RANGED)
	{
		...
		if(mode != ALIEN_FLEEING 
			&& mode != ALIEN_LANDING_AT_HOME
			&& mode != ALIEN_LANDED_HEALING
			&& (mode != ALIEN_LANDED_DEPOSITING || rand()%100 < 2)
			&& (mode != ALIEN_LANDED_WITHDRAWING || rand()%100 < 5)
			&& pending_mode != ALIEN_LANDED_HEALING
			&& pending_mode != ALIEN_LANDED_DEPOSITING
			&& pending_mode != ALIEN_LANDED_WITHDRAWING
			&& mode != ALIEN_FINDING_BIRTHPLACE
			&& mode != ALIEN_GIVING_BIRTH)
		{
			return true;
		}
	}
...
bool AlienOrganism::ModeSupportsSquadRecruitment(int mode)
{
	if(mode==ALIEN_LANDED_RESTING 
		|| mode==ALIEN_HUNTING
		||	mode==ALIEN_SCAVENGING
		|| mode==ALIEN_ATTACKING_MELEE) return true;
...
bool AlienOrganism::ModeSupportsSquadEnlistment(int mode, int pending_mode, int recruiter_mode)
{
	...
	if(mode==ALIEN_LANDED_RESTING
		|| (mode==ALIEN_GOING_HOME && pending_mode==ALIEN_LANDED_RESTING)
		|| mode==ALIEN_HUNTING
		||	mode==ALIEN_SCAVENGING
		|| mode==ALIEN_ATTACKING_MELEE) return true;
...
{
	if(!(memory->squad_flags & ALIEN_LEADER)) return;
...
{
	if(!(memory->squad_flags & ALIEN_SQUADIE)) return true;
...
bool AlienOrganism::Flee(int me, bool force_flee/*=false*/)
{
	...
	if(memory->squad_flags & ALIEN_SQUADIE)
	{
		if(!force_flee)
		{
			...
			if((rand()%100) < ALIEN_SQUADIE_FLEE_SAVING_THROW) return false;
		}
	...
	if(memory->squad_flags & ALIEN_LEADER)
	{
		if(!force_flee)
		{
			...
			if((rand()%100) < ALIEN_LEADER_FLEE_SAVING_THROW) return false;
		}
...
bool AlienOrganism::WillJoinLeader(int me, int leader_handle, int priority)
{
	...
	if(!ModeSupportsSquadEnlistment(memory->mode,memory->pending_mode,recruiter_mode)) return false;
...
int AlienOrganism::FindTeammateID(int tm_handle)
{
	for(int i = 0; i < memory->num_teammates; i++)
	{
		if(memory->teammate[i].handle == tm_handle) return i;
	}
...
void AlienOrganism::SetSquadieFormationGoal(int me)
{
	...
	AI_AddGoal(me, AIG_GET_TO_POS, 1, 1.0f, -1, GF_ORIENT_VELOCITY | GF_KEEP_AT_COMPLETION, &memory->squad_goal_pos, memory->squad_goal_room);
...
void AlienOrganism::SetWanderGoal(int me)
{
	...
	if(memory->dest_object_handle != OBJECT_HANDLE_NONE)
	{
		...
		if(type != OBJ_NONE)
		{
			AI_AddGoal(me, AIG_GET_TO_OBJ, 2, 1.0f, ALIEN_GUID_GOT_TO_DEST_OBJ, GF_ORIENT_VELOCITY | GF_USE_BLINE_IF_SEES_GOAL | GF_KEEP_AT_COMPLETION | GF_NONFLUSHABLE, memory->dest_object_handle);
...
bool AlienOrganism::IsNearFire(int me)
{
	return false;
}
...
bool AlienOrganism::IsOnFire(int me)
{
	return(Obj_IsEffect(me, EF_NAPALMED));
}
...
void AlienOrganism::ProjectBeam(int me, int pos_handle, vector *start_pos, int start_room, vector *beam_dir, matrix *orient, int beam_flag)
{
	ray_info ray;
	int flags, fate;
...
void AlienOrganism::UpdateEnergyBeams(int me)
{
	...
	if(!memory->energy_beams_on) return;
...
void AlienOrganism::SetMaxSpeed(int me, float speed)
{
...
void AlienOrganism::SetCircleDist(int me, float dist)
{
	AI_Value(me, VF_GET, AIV_F_CIRCLE_DIST, &dist);
}
...
void AlienOrganism::SetMode(int me, char mode)
{
	...
	switch(mode)
	{
		...
		case ALIEN_LANDING_AT_HOME:
		{
			...
			AI_AddGoal(me, AIG_GET_TO_POS, 2, 100.0f, ALIEN_GUID_LANDED, GF_NONFLUSHABLE | GF_KEEP_AT_COMPLETION | GF_ORIENT_SCRIPTED, &memory->home_pos, memory->home_room);
	...
	if((memory->squad_flags & ALIEN_SQUADIE) && !(memory->squad_flags & ALIEN_BROKEN) && ModeSupportsSquadMovement(mode)) {
...
bool AlienOrganism::FindHome(int me)
{
	...
	while(!home_found && num_attempts < MAX_HOME_FINDING_ATTEMPTS)
	{
		...
		if(fate == HIT_WALL)
		{
			...
			if(rand()%100 > 50) memory->home_fvec = -memory->home_fvec;
...
void AlienOrganism::DoSquadieFrame(int me)
{
	...
	if(!ModeSupportsSquadMovement(memory->mode)) return;

	// If squad is currently broken, get outta here
	if(memory->squad_flags & ALIEN_BROKEN) return;
...
void AlienOrganism::DoFrame(int me)
{
	...
	*/
	...
	if(memory->next_generic_check_time <= Game_GetTime())
	{
		...
		if(!IsLandedMode(memory->mode) && memory->mode!=ALIEN_LANDING_AT_HOME && memory->pending_mode!=ALIEN_LANDED_HEALING)
		{
		...
		if(memory->mode!=ALIEN_FLEEING && memory->mode!=ALIEN_LANDING_AT_HOME && memory->mode!=ALIEN_LANDED_HEALING && memory->pending_mode!=ALIEN_LANDED_HEALING)
		{
			if(IsOnFire(me))
			{
				// If we're on fire, there's a good chance that we will try to flee
				if((rand()%100) < ALIEN_ON_FIRE_FLEE_CHANCE) Flee(me);
			}
			else if(IsNearFire(me))
			{
				// If we're near fire, there's a chance that we will try to flee
				if((rand()%100) < ALIEN_NEAR_FIRE_FLEE_CHANCE) Flee(me);
			}
		}
	...
	switch(memory->mode)
	{
		...
		case ALIEN_LANDED_HEALING:
		{
			...
			if(!memory->energy_beams_on) CalcEnergyBeamPositions(me);
...
bool AlienOrganism::DoNotify(int me, tOSIRISEventInfo *data)
{
	...
	else if(data->evt_ai_notify.notify_type == AIN_FIRED_WEAPON)
	{
		...
		if(memory->energy_charges < 0.0f) memory->energy_charges = 0.0f;
	}
...
void AlienOrganism::DoDamage(int me, tOSIRISEVTDAMAGED *damage_data)
{
	...
	*/
	...
	if ((damage_data->damage_type == GD_ENERGY || damage_data->damage_type == GD_ELECTRIC) && Game_GetTime() >= memory->next_special_damage_time)
	{
...
void HeavyTrooper::SetMode(int me, char mode)
{
	...
	switch(mode)
	{
		...
		case HT_ARMORED_ATTACK:
		{
			...
			AI_AddGoal(me, AIG_GET_TO_OBJ, 1, 1.0f, -1, GF_ORIENT_VELOCITY | GF_OBJ_IS_TARGET | GF_USE_BLINE_IF_SEES_GOAL | GF_KEEP_AT_COMPLETION | GF_NONFLUSHABLE, OBJECT_HANDLE_NONE);
		...
		case HT_ARMORED_CIRCLE_BACK:
		{
			...
			int g_index = AI_AddGoal(me, AIG_GET_AROUND_OBJ, ACTIVATION_BLEND_LEVEL, 1.0f, -1, GF_OBJ_IS_TARGET | GF_KEEP_AT_COMPLETION | GF_NONFLUSHABLE, OBJECT_HANDLE_NONE);
...
void HeavyTrooper::DoFrame(int me)
{
	...
	if(memory->mode == HT_ATTACK_NORMAL || memory->mode == HT_PREPARING_GRENADE || memory->mode == HT_FIRING_GRENADE || memory->mode == HT_RECOVERING)
	{
	...
	switch(memory->mode)
	{
		case HT_ATTACK_NORMAL:
		{
			...
			{
				memory->next_grenade_check_time = Game_GetTime() + HT_GRENADE_CHECK_INTERVAL + ((float)rand()/(float)RAND_MAX)*HT_GRENADE_CHECK_VARIANCE;
...
// Process all AI Notify events
bool HeavyTrooper::DoNotify(int me, tOSIRISEventInfo *data)
{
	return true;
}
...
// Sets the trooper's max speed
void Lifter::SetMaxSpeed(int me, float speed)
{
	AI_Value(me, VF_SET, AIV_F_MAX_SPEED, &speed);
}
...
bool Lifter::OkToPull(int me, bool initial_check/*=true*/)
{
	...
	if(initial_check)
	{
		...
		// Wait until the current anim type is alert, then return to normal
		if(type!=AS_ALERT) return false;
	}

	// Make sure we still have a target
	AI_Value(me, VF_GET, AIV_I_TARGET_HANDLE, &target_handle);
	if(target_handle == OBJECT_HANDLE_NONE) return false;

	// Make sure it's a pullable target
	int type;
	Obj_Value(target_handle, VF_GET, OBJV_I_TYPE, &type);
	if(type != OBJ_PLAYER) return false;

	// If it's not the initial pull, make sure the target hasn't changed
	if(!initial_check && target_handle != memory->pull_target) return false;

	// Make sure the target isn't cloaked
	if(qObjectCloakTime(target_handle) != 0.0f) return false;

	// check distance to target
	float dist;
	AI_Value(me, VF_GET, AIV_F_DIST_TO_TARGET, &dist);
	if(dist > LIFTER_MAX_PULLING_DIST) return false;

	// Only check min distance if it's the initial decision check (whether to start pulling)
	if(initial_check && dist < LIFTER_MIN_PULLING_DIST) return false;
	...
	if(fov_angle < 0.7f) return false;
...
void Lifter::PullTarget(int me)
{
	...
	// Make sure we still have a target
	AI_Value(me, VF_GET, AIV_I_TARGET_HANDLE, &target_handle);
	if(target_handle == OBJECT_HANDLE_NONE || target_handle != memory->pull_target) return;
...
void Lifter::UpdateTractorBeam(int me)
{
	...
	aLightningCreate(memory->pull_source_prop,memory->pull_target_prop,next_duration,1.0f,4,tractor_beam_effect_id,next_duration,1,255,255,255,false);
}


// All the controls that will be taken away while being force pulled
#define PLAYER_CONTROL_MASK		(PLYV_CMASK_FORWARD|PLYV_CMASK_REVERSE|PLYV_CMASK_LEFT|PLYV_CMASK_RIGHT|PLYV_CMASK_UP|PLYV_CMASK_DOWN|PLYV_CMASK_AFTERBURNER)
...
void Lifter::ConfineTarget(int me)
{
	...
	// Make sure we still have a target
	AI_Value(me, VF_GET, AIV_I_TARGET_HANDLE, &target_handle);
	if(target_handle == OBJECT_HANDLE_NONE) return;
...
void Lifter::ReleaseTarget(int me)
{
	...
	memory->next_pull_check_time = Game_GetTime() + LIFTER_NEXT_PULL_DELAY + (((float)rand()/(float)RAND_MAX) * LIFTER_NEXT_PULL_VARIANCE);
...
void Lifter::SwitchToAlert(int me)
{
	Obj_SetCustomAnim(me, 10, 12, 0.3f, AIAF_NOTIFY, -1, AS_ALERT);
}
...
void Lifter::UpdateLiftBeam(int me)
{
	...
	if(memory->next_lift_beam_time <= Game_GetTime())
	{
		...
		{
			aLightningCreate(me,attached_handle,next_duration,2.0f,2,boss_heal_effect_id,next_duration,1,255,255,255,false);
		}
	}
}
...
void AlienBoss::SetMaxSpeed(int me, float speed)
{
	AI_Value(me, VF_SET, AIV_F_MAX_SPEED, &speed);
}
...
// Checks to see if alien is currently on fire
bool AlienBoss::IsOnFire(int me)
{
	return(Obj_IsEffect(me, EF_NAPALMED));
}
...
void AlienBoss::SetHideRoomGoal(int me)
{
	...
	Room_Value(roomnum,VF_GET,RMSV_V_PATH_PNT,&pos,0);
	AI_AddGoal(me,AIG_GET_TO_POS,2,DEFAULT_INFLUENCE,AB_GUID_GOT_TO_HIDE_ROOM,GF_ORIENT_VELOCITY|GF_KEEP_AT_COMPLETION,&pos,roomnum);
...
void AlienBoss::AbortCurrentMode(int me)
{
	switch(memory->mode)
	{
		...
		{
			// Play the takeoff anim and tell it to go to alert next
			Obj_SetCustomAnim(me, AB_TAKEOFF_START_FRAME, AB_TAKEOFF_END_FRAME, AB_TAKEOFF_ANIM_TIME, AIAF_NOTIFY, -1, AS_ALERT);		
		}
		break;
...
void AlienBoss::UpdateHealingEnergyBeams(int me)
{
	...
	if(memory->next_update_beam_time <= Game_GetTime())
	{
		...
		aLightningCreate(memory->tail_pos_handle,memory->nest_handle,next_duration,1.0f,2,boss_heal_effect_id,next_duration,1,255,255,255,false);
...
void AlienBoss::UpdateDamageEffect(int me)
{
	...
	if(memory->next_damage_effect_time <= Game_GetTime())
	{
		...
		if(curr_damage > memory->damage_threshold) curr_damage = memory->damage_threshold;
...
{
	if(mode==AB_WANDERING 
		|| mode==AB_GOING_TO_NEST 
		|| mode==AB_LANDING_AT_NEST 
...
void AlienBoss::SetMode(int me, char mode)
{
	...
	switch(mode)
	{
		...
		case AB_LANDING_AT_NEST:
		{
			...
			// Make him land at his home point
			AI_AddGoal(me, AIG_GET_TO_POS, 2, 100.0f, AB_GUID_LANDED, GF_NONFLUSHABLE | GF_KEEP_AT_COMPLETION | GF_ORIENT_SCRIPTED, &memory->home_pos, memory->home_room);
			AI_SetGoalCircleDist(me, 2, 0.0f);
		...
		case AB_FLEEING:
		{
			...
			if(memory->fire_flee_chance < 0.0f) memory->fire_flee_chance = 0.0f;
		}
		break;
...
void AlienBoss::DoCustomLookups(void)
{
	...
	for(j=0;j<AB_NUM_WANDER_ROOMS;j++)
	{
		...
		if(memory->wander_rooms[j] < 0) mprintf(0,"Alien Boss Error: Room not found - %s",AB_WanderRoomNames[j]);
	}
}
...
bool AlienBoss::OkToStartSpecialAttack(int me)
{
	...
	float dist;
	AI_Value(me, VF_GET, AIV_F_DIST_TO_TARGET, &dist);
	if(dist > AB_MAX_SPECIAL_ATTACK_DIST) return false;
...
bool AlienBoss::DoStingAttack(int me)
{
	...
	float dist;
	AI_Value(me, VF_GET, AIV_F_DIST_TO_TARGET, &dist);
	if(dist > AB_MAX_STING_DIST) return false;
	...
	aLightningCreate(target_handle,memory->tail_pos_handle,0.4f,1.0f,3,transfer_effect_id,0.4f,1,255,255,255,false);
...
void AlienBoss::DoFrame(int me)
{
	...
	switch(memory->mode)
	{
			...
			if(Game_GetTime() >= memory->next_activity_time) 
			{
				...
				if(Game_GetTime() >= memory->next_special_attack_time) 
				{
					if(OkToStartSpecialAttack(me))
					{
						...
						memory->next_special_attack_time = Game_GetTime() + AB_SPECIAL_ATTACK_DELAY + ((float)rand()/(float)RAND_MAX)*AB_SPECIAL_ATTACK_VARIANCE;
		...
		case AB_MELEE_ATTACK:
		{
			...
			{
				memory->next_melee_attack_time = Game_GetTime() + AB_MELEE_ATTACK_DELAY + ((float)rand()/(float)RAND_MAX)*AB_MELEE_ATTACK_VARIANCE;
...
void SecurityCamera::SetMode(int me, char mode)
{
	...
	switch(mode)
	{
		case SC_PANNING:
		{
			...
			if(curr_anim_frame > SC_MAX_FRAME) curr_anim_frame = SC_MAX_FRAME;
	...
	switch(memory->mode)
	{
		...
		case SC_TRACKING:
		{
			...
			if(Game_GetTime() >= memory->next_activity_time) 
			{
				...
				dir.y = 0.0f;
				if((dir.x + dir.y + dir.z) == 0.0f) break;
				vm_VectorNormalize(&dir);

				local_fvec.x = 0.0f; local_fvec.y =0.0f; local_fvec.z = 1.0f;
				dot = dir * local_fvec;
				...
				if(frame_offset > SC_MAX_FRAME_OFFSET) frame_offset = SC_MAX_FRAME_OFFSET;
				...
				if(dir.x < 0.0f) frame_offset = -frame_offset;
...
void CrowdControl::DoFrame(int me)
{
	...
	if(memory->follow_handle == OBJECT_HANDLE_NONE) return;
