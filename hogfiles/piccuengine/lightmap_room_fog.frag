...
void main()
{
	...
	color = mix(vec4(basecolor.rgb * lmcolor.rgb, 1.0), vec4(room.fog_color.rgb, 1.0f), fog_amount) * vec4(outlight, outlight, outlight, 1.0);
	post_mask = vec4(0.0, fog_amount, 0.0, float(clamp(ao_class_value, 0, 255)) / 255.0);
}
