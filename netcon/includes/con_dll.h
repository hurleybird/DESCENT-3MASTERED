...
typedef void( *DoMessageBox_fp) (const char *title, const char *msg, int type, ddgr_color title_color, ddgr_color text_color);
DoMessageBox_fp DLLDoMessageBoxFP;
inline void DLLDoMessageBox (const char *title, const char *msg, int type, ddgr_color title_color = GR_WHITE, ddgr_color text_color = GR_WHITE)
{
...
typedef void *(*HotSpotCreate_fp) (void *parentwin, int id, int key, void * txtitemoff, void * txtitemon, int x, int y, int w, int h, int flags);
...
typedef void *(*UIConsoleGadgetCreate_fp) (void * parentid, int id, int x, int y, int font, int cols, int rows, int flags);
...
int StartMultiplayerGameMenu ()
{
	...
	int a;
	for(a=0;a<MAX_DLLS;a++) dll_txt_items[a] = NULL;
	char str[100];
	int exit_menu=0;
	int cury=40;
	int ret=0;
	...
	void * start_title = DLLTextCreate(main_wnd,start_text,0,cury,UIF_CENTER); cury+=35;
	
	// Mission name
	void * mission_text = DLLTextCreate(main_wnd,game_name_text,0,cury,UIF_CENTER); cury+=15;
	void * mission_name_edit = DLLEditCreate(main_wnd,id++,10,cury,300,15,UIF_CENTER); cury+=18;
	cury+=30;

	// mission name
	void * game_name = DLLTextCreate(main_wnd,mission_name_text,45,cury,0); cury+=15;
	...
	// Script box
	void * script_name = DLLTextCreate(main_wnd,script_name_text,370,cury,0); cury+=18;
	void * script_list = DLLListCreate(main_wnd,id++,360,cury,200,100,0); cury+=118;
	cury+=40;

	// Start button
	int start_button=UID_OK;
	void * start_hs = DLLHotSpotCreate(main_wnd,UID_OK,KEY_ENTER,start_game_off_text,start_game_on_text, 10,cury, 180, 30,UIF_FIT|UIF_CENTER); cury+=18;

	// Option button
	int option_button=id;
	void * option_hs = DLLHotSpotCreate(main_wnd,id++,KEY_O,multiplayer_opts_off_text,multiplayer_opts_on_text, 10,cury, 180, 30,UIF_FIT|UIF_CENTER); cury+=18;

	// cancel button
		
	int save_button = id;
	void * save_hs = DLLHotSpotCreate(main_wnd,id++,0,save_settings_txt_off,save_settings_txt_on, 10,cury, 180, 30,UIF_FIT|UIF_CENTER); cury+=18;
	int load_button = id;
	void * load_hs = DLLHotSpotCreate(main_wnd,id++,0,load_settings_txt_off,load_settings_txt_on, 10,cury, 180, 30,UIF_FIT|UIF_CENTER);cury+=18; 
	
	int cancel_button=UID_CANCEL;
	void * cancel_hs = DLLHotSpotCreate(main_wnd,UID_CANCEL,KEY_ESC,exit_off_text,exit_on_text, 10,cury, 180, 30,UIF_FIT|UIF_CENTER); 
...
// Bash some values to default
	...
	if(DLLMultiLoadSettings(dftset))
	{
		...
		if(mi) DLLListSelectItem(list_1,mi->ti);
#endif
		for(index=0;index<dllcount;index++)
		{
...
//			int teams = 1;
	...
	for(a=0;a<MAX_DLLS;a++) if(dll_txt_items[a]) DLLRemoveUITextItem(dll_txt_items[a]);
	RemoveAllMsnItems();
	//cleanup
#ifdef DEMO
